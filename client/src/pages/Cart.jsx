...
const Cart = () => {
  ...
  try {
    ...
    // STRIPE FLOW (DEMO MODE)
    if (data.success && data.demo) {
      // Demo mode - show message without creating order
      toast.success("Online payment is in demo mode only", { duration: 4000 });
      toast("No real payment will be processed", { icon: "ℹ️", duration: 4000 });
      // Stay on cart page - don't redirect or clear cart
    } else if (data.success && data.url) {
      // Real payment flow (if enabled in future)
          ...
          <div className="flex items-start">
            ...
            <div className="ml-3">
              <p className="text-sm text-yellow-700 font-medium">
                Demo Mode: Online payment is disabled
              </p>
              <p className="mt-1 text-sm text-yellow-600">
                This is a demonstration only. No real payment will be processed and no order will be created.
              </p>
            </div>
          </div>
        </div>
